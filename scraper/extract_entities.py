...
MIN_NAME_LENGTH = {
    "person": 3,   # "Ли" → ok (2 букв мало), но "Маск" → ok
    "org": 2,
    "location": 2,
}
...
def clean_entity_name(raw_name: str, entity_type: str) -> str | None:
    ...
    # ═══════════════════════════════════════════════════════════
    # Агрессивная очистка мусорных префиксов
    # Примеры: "делАрман Исетов" → "Арман Исетов"
    #          "наТАСС" → "ТАСС"
    #          "казахстанакасым-жомарт Токаев" → "Касым-Жомарт Токаев"  [будет удалён как мусор]
    #          "государстваКасым-Жомарт Токаев" → "Касым-Жомарт Токаев"
    ...
    # ═══════════════════════════════════════════════════════════
    # Ищем все переходы строчная→заглавная (без пробела)
    # Отрезаем всё до ПОСЛЕДНЕГО такого перехода если мусор слипся
    if re.search(r'[а-яёa-z][А-ЯЁA-Z]', name):
        # Находим позицию: ищем последний переход lowercase→Uppercase
        # но берём ПЕРВЫЙ, чтобы не потерять составные имена типа "Касым-Жомарт Токаев"
        matches = list(re.finditer(r'[а-яёa-z]([А-ЯЁA-Z])', name))
        if matches:
            first_match = matches[0]
            cut_pos = first_match.start(1)  # позиция заглавной буквы
            prefix = name[:cut_pos]
            # Проверяем — если префикс не содержит пробелов, это мусор
            if ' ' not in prefix:
                name = name[cut_pos:]

    # Финальная зачистка: если после очистки осталось что-то типа "Токаевподписал" —
    # ищем мусорный суффикс (строчные после заглавного слова без пробела)
    # "Касым-Жомарт Токаевподписал" → отбрасываем (слишком сложно очистить)
    words = name.split()
    cleaned_words = []
    for w in words:
        # Если слово содержит переход Uppercase→lowercase→Uppercase внутри (без дефиса),
        # это слипшиеся слова — берём только начало
        if re.search(r'[а-яё][а-яё]+[А-ЯЁ]', w) and '-' not in w:
            # Пример: "Токаевподписал" — это мусор, пропускаем всё слово?
            # Нет — берём до перехода: "Токаев" + "подписал" → оставляем "Токаев"... 
            # Лучше просто отбросить — слишком ненадёжно
            return None
    ...
    # Для персон — НЕ требуем 2 слова. Фамилии-одиночки (Токаев, Путин, Трамп) — валидны.
    # Но каждое слово должно начинаться с заглавной
